...
import com.alex.csvhelper.validator.FieldValidator;
...
public class CsvReader<T extends CsvBean> {
  ...
  public List<T> getBeans(int pageSize) throws CsvReadException, IOException {
    ...
    if (logger.isDebugEnabled())
      logger.debug("getBeans(" + pageSize + ")耗时：" + (System.currentTimeMillis() - sysTime) + "ms。");
    return beans;
  }

  private T convert(int lineNumber, String line) throws CsvReadException {
    T bean = ReflectUtil.reflectBean(titleMap, beanClass, line);
